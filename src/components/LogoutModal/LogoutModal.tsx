...
interface ILogoutRef {
 logoutRef: React.RefObject<HTMLDivElement>
}

export const LogoutModal = ({logoutRef}:ILogoutRef) => {
  const {data: session} = useSession();

  
  return (
    <div className="z-50 w-[200px] top-16 right-6 absolute bg-slate-100 shadow-lg rounded-md border p-2 flex flex-col h-auto" ref={logoutRef}>
