...
export const NotificationItemCard = ({
  ...
  return (
    <div
      className={` w-full bg-slate-100 shadow-md rounded-sm px-4 py-2 text-cyan-700 flex flex-col gap-1 ${
    ...
    >
      ...
      <div className="w-full flex items-center justify-between mt-4">
        ...
        <button
          ...
          onClick={trashEmail}
          className={`${
            session?.user.role === "user" && "disabled:cursor-not-allowed"
          }`}
          disabled={session?.user.role === "user"}
        >
          <IoMdTrash
            size={20}
            className={`${session?.user.role === "user" && "text-zinc-400"}`}
          />
        </button>
      </div>
    </div>
